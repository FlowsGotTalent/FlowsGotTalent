...
function spawnParticles(pos, count, color, scale) {
    for (let i = 0; i < count; i++) {
        ...
        TweenMax.to(mesh.position, speed, {
            x: targetX, y: targetY, z: targetZ, delay: Math.random() * .1, ease: Power2.easeOut, onComplete: () => {
                scene.remove(mesh)
            }
        })
    }
...
function loop() {
    ...
    if (game.status == 'playing') {
        ...
        if (game.level > 1) {
            parent.postMessage({name: "gameEvent", score: game.level, nextRound: 8}, "*")
        }
...
function addLife() {
    ...
    parent.postMessage({name: "gameEvent", score: game.level, resetTime: true, addTime: 60}, "*")
}
...
class UI {
    ...
    hideReplay() {
        ...
        parent.postMessage({name: "gameEvent", score: game.level, resetTime: true}, "*")
    }
...
function resetMap() {
    ...
    airplane.equipWeapon(new SimpleGun())
    // 7airplane.equipWeapon(new DoubleGun())
    // airplane.equipWeapon(new BetterGun())
}
